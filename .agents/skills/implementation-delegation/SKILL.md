...
- (あるときだけ)issue に追記した内容を1行 —— 委任先に伝えることはすべて issue へ書いてから出力する(§3)。`/implement-rdi` に渡るのは issue 番号と上の2行だけ
...
- **issue 本文が発注書。** 完了条件・触ってよいファイル・不変条件(テストファイル名)を issue に書いてから投げる。薄ければプロンプトではなく issue を書き足し、選定で出た特記(ponytail-review への釘、レビューで見る面)も issue に追記する —— `/implement-rdi` は issue 番号と決定しか受け取らない。別立ての発注書ファイルは、issue に書くべきでないその回限りの事情(スコープ制限 / 適用スキルの絞り込み / ADR が正本のときの読む順序)だけ。
...
床に当たるのは framework の不変条件: 観測語彙と Core 語彙の disjoint 強制、bridge rule の `theory/bridge/` 限定、`parse` が自由散文を受けないこと、ADR に記録済みの決定。理論スライスは仕様が issue の grill で固まっていれば主力ティア / high、現象選択が未了なら委任しない(`/implement-rdi` の Theory slice stages を先に)。実例は PR が溜まってから足す。
